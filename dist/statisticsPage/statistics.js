...
initStatisticsPage = function initStatisticsPage() {
    Promise.all([bg.storage.getInterceptDateList(), bg.storage.getExerciseTimeList()]).then(function (response) {
        var counters = bg.interception.calcInterceptData(response[0].tds_interceptDateList);
